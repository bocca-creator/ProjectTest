export const translations = {
  en: {
    // Navigation
    nav: {
      home: "Home",
      rules: "Rules", 
      faq: "FAQ",
      about: "About",
      team: "Team",
      contact: "Contact",
      login: "Login",
      register: "Register",
      logout: "Logout",
      profile: "Profile"
    },
    
    // Hero Section
    hero: {
      welcome: "Welcome to",
      subtitle: "GAMING COMMUNITY",
      description: "Join the ultimate cyberpunk gaming experience. Connect with players, compete in tournaments, and dominate the digital battlefield.",
      joinCommunity: "Join Community",
      viewServers: "View Servers",
      activePlayersLabel: "ACTIVE PLAYERS",
      onlineNowLabel: "ONLINE NOW", 
      serverUptimeLabel: "SERVER UPTIME"
    },
    
    // Project Rules
    rules: {
      title: "Project Rules",
      respectMembers: "Respect all community members",
      noCheating: "No cheating or exploiting",
      fairPlay: "Follow fair play guidelines", 
      civilCommunication: "Keep communications civil",
      reportSuspicious: "Report suspicious activities",
      readFullRules: "Read Full Rules"
    },
    
    // Discord Section
    discord: {
      title: "Discord Server",
      onlineLabel: "online",
      membersLabel: "members",
      activeChannels: "Active Channels",
      recentActivity: "Recent Activity"
    },
    
    // Announcements
    announcements: {
      title: "Project Updates & Announcements",
      readMore: "Read More",
      comments: "Comments",
      author: "Author",
      timestamp: "Posted"
    },
    
    // Reviews
    reviews: {
      title: "Community Reviews",
      likes: "likes",
      rating: "Rating"
    },
    
    // Authentication
    auth: {
      loginTitle: "Login to Your Account",
      registerTitle: "Create New Account",
      email: "Email",
      username: "Username", 
      displayName: "Display Name",
      password: "Password",
      confirmPassword: "Confirm Password",
      loginButton: "Login",
      registerButton: "Register",
      forgotPassword: "Forgot Password?",
      switchToRegister: "Don't have an account? Register",
      switchToLogin: "Already have an account? Login",
      logout: "Logout",
      profile: "Profile",
      settings: "Settings"
    },
    
    // Profile
    profile: {
      title: "User Profile",
      personalInfo: "Personal Information",
      gameStats: "Game Statistics",
      preferences: "Preferences",
      security: "Security",
      bio: "Bio",
      joinedDate: "Member Since",
      lastLogin: "Last Login",
      loginCount: "Total Logins",
      steamProfile: "Steam Profile",
      customThemes: "Custom Themes",
      createTheme: "Create Theme"
    },
    
    // Theme Editor
    themes: {
      title: "Theme Editor",
      createNew: "Create New Theme",
      myThemes: "My Themes", 
      publicThemes: "Public Themes",
      themeName: "Theme Name",
      themeDescription: "Description",
      category: "Category",
      makePublic: "Make Public",
      saveTheme: "Save Theme",
      applyTheme: "Apply Theme",
      deleteTheme: "Delete Theme",
      colors: "Colors",
      background: "Background",
      text: "Text",
      accent: "Accent",
      preview: "Preview"
    },
    
    // Settings
    settings: {
      title: "Settings",
      language: "Language",
      theme: "Theme",
      notifications: "Notifications",
      privacy: "Privacy",
      saveChanges: "Save Changes",
      resetToDefault: "Reset to Default"
    },
    
    // Common
    common: {
      save: "Save",
      cancel: "Cancel",
      delete: "Delete",
      edit: "Edit",
      close: "Close",
      loading: "Loading...",
      error: "Error",
      success: "Success",
      warning: "Warning",
      confirm: "Confirm",
      yes: "Yes",
      no: "No",
      search: "Search",
      filter: "Filter",
      sort: "Sort",
      page: "Page",
      of: "of",
      total: "Total",
      back: "Back",
      next: "Next",
      previous: "Previous"
    }
  },
  
  uk: {
    // Navigation 
    nav: {
      home: "Головна",
      rules: "Правила",
      faq: "Питання",
      about: "Про нас",
      team: "Команда", 
      contact: "Контакти",
      login: "Увійти",
      register: "Реєстрація",
      logout: "Вийти",
      profile: "Профіль"
    },
    
    // Hero Section
    hero: {
      welcome: "Ласкаво просимо до",
      subtitle: "ІГРОВА СПІЛЬНОТА",
      description: "Приєднуйтесь до найкращого кіберпанк ігрового досвіду. Спілкуйтесь з гравцями, беріть участь у турнірах та домінуйте на цифровому полі бою.",
      joinCommunity: "Приєднатися",
      viewServers: "Переглянути сервери",
      activePlayersLabel: "АКТИВНІ ГРАВЦІ",
      onlineNowLabel: "ОНЛАЙН ЗАРАЗ",
      serverUptimeLabel: "ЧАС РОБОТИ СЕРВЕРА"
    },
    
    // Project Rules
    rules: {
      title: "Правила Проекту",
      respectMembers: "Поважайте всіх учасників спільноти",
      noCheating: "Заборонено використання читів або експлойтів",
      fairPlay: "Дотримуйтесь принципів чесної гри",
      civilCommunication: "Підтримуйте цивілізоване спілкування",
      reportSuspicious: "Повідомляйте про підозрілу активність",
      readFullRules: "Переглянути повні правила"
    },
    
    // Discord Section
    discord: {
      title: "Discord Сервер", 
      onlineLabel: "онлайн",
      membersLabel: "учасників",
      activeChannels: "Активні канали",
      recentActivity: "Недавня активність"
    },
    
    // Announcements
    announcements: {
      title: "Оновлення та Оголошення Проекту",
      readMore: "Читати далі",
      comments: "Коментарі",
      author: "Автор",
      timestamp: "Опубліковано"
    },
    
    // Reviews
    reviews: {
      title: "Відгуки Спільноти",
      likes: "вподобань",
      rating: "Рейтинг"
    },
    
    // Authentication
    auth: {
      loginTitle: "Увійти до вашого акаунту",
      registerTitle: "Створити новий акаунт",
      email: "Електронна пошта",
      username: "Ім'я користувача",
      displayName: "Відображуване ім'я", 
      password: "Пароль",
      confirmPassword: "Підтвердіть пароль",
      loginButton: "Увійти",
      registerButton: "Зареєструватися",
      forgotPassword: "Забули пароль?",
      switchToRegister: "Немає акаунту? Зареєструватися",
      switchToLogin: "Вже є акаунт? Увійти",
      logout: "Вийти",
      profile: "Профіль",
      settings: "Налаштування"
    },
    
    // Profile
    profile: {
      title: "Профіль користувача",
      personalInfo: "Особиста інформація",
      gameStats: "Ігрова статистика",
      preferences: "Налаштування",
      security: "Безпека",
      bio: "Про себе",
      joinedDate: "Учасник з",
      lastLogin: "Останній вхід",
      loginCount: "Загальна кількість входів",
      steamProfile: "Steam профіль",
      customThemes: "Користувацькі теми",
      createTheme: "Створити тему"
    },
    
    // Theme Editor
    themes: {
      title: "Редактор тем",
      createNew: "Створити нову тему",
      myThemes: "Мої теми",
      publicThemes: "Публічні теми",
      themeName: "Назва теми",
      themeDescription: "Опис",
      category: "Категорія",
      makePublic: "Зробити публічною",
      saveTheme: "Зберегти тему",
      applyTheme: "Застосувати тему",
      deleteTheme: "Видалити тему",
      colors: "Кольори",
      background: "Фон",
      text: "Текст",
      accent: "Акцент",
      preview: "Попередній перегляд"
    },
    
    // Settings
    settings: {
      title: "Налаштування",
      language: "Мова",
      theme: "Тема",
      notifications: "Сповіщення",
      privacy: "Конфіденційність",
      saveChanges: "Зберегти зміни",
      resetToDefault: "Скинути до початкових"
    },
    
    // Common
    common: {
      save: "Зберегти",
      cancel: "Скасувати",
      delete: "Видалити", 
      edit: "Редагувати",
      close: "Закрити",
      loading: "Завантаження...",
      error: "Помилка",
      success: "Успішно",
      warning: "Попередження",
      confirm: "Підтвердити",
      yes: "Так",
      no: "Ні", 
      search: "Пошук",
      filter: "Фільтр",
      sort: "Сортування",
      page: "Сторінка",
      of: "з",
      total: "Всього",
      back: "Назад",
      next: "Далі",
      previous: "Попередня"
    }
  }
};